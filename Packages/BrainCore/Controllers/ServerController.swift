...
final class ServerController: ObservableObject {
    ...
    private var serverActor: BrainOSServer?
    ...
    func startServer() async {
        ...
        do {
            ...
            print("[BrainOS] Starting NIO server on \(bindHost):\(configuration.port)")
            ...
            print("[BrainOS] NIO server started successfully on port \(configuration.port)")
        } catch {
    ...
    func stopServer() async {
        ...
        print("[BrainOS] Stopping NIO server...")
        ...
        print("[BrainOS] Server stopped successfully")
    }
    ...
    func ensureShutdown() async {
        ...
        print("[BrainOS] Ensuring NIO server shutdown before app termination")
        isRunning = false
        ...
        print("[BrainOS] Server shutdown completed")
    }
    ...
    private func handleServerError(_ error: Error) {
        print("[BrainOS] Failed to start server: \(error)")
        isRunning = false
        ...
        if let group = eventLoopGroup {
            await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
                group.shutdownGracefully { error in
                    if let error {
                        print("[BrainOS] Error shutting down EventLoopGroup: \(error)")
                    }
                    continuation.resume()
                }
            }

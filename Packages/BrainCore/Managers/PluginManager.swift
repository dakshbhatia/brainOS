...
final class PluginManager {
    ...
    private func loadPluginWithError(at url: URL) -> Result<LoadedPlugin, PluginLoadError> {
        ...
        guard let handle = dlopen(url.path, Int32(flags)) else {
            ...
            print("[BrainOS] dlopen failed for \(url.path): \(errorMsg)")
            return .failure(PluginLoadError(message: errorMsg))
        }

        // Look for the entry point
        guard let sym = dlsym(handle, "BrainOS_plugin_entry") else {
            let errorMsg = "Missing plugin entry point (BrainOS_plugin_entry)"
            print("[BrainOS] \(errorMsg) in \(url.lastPathComponent)")
            dlclose(handle)
            return .failure(PluginLoadError(message: errorMsg))
        }

...
public final class RemoteProviderManager: ObservableObject {
    ...
    public func connect(providerId: UUID) async throws {
        ...
        do {
            ...
            print("[BrainOS] Remote Provider '\(provider.name)': Connected with \(models.count) models")
            ...
            print("[BrainOS] Remote Provider '\(provider.name)': Connection failed - \(error)")
    ...
    public func disconnect(providerId: UUID) {
        ...
        if let provider = configuration.provider(id: providerId) {
            print("[BrainOS] Remote Provider '\(provider.name)': Disconnected")
        }
    ...
    public func connectEnabledProviders() async {
        for provider in configuration.enabledProviders {
            do {
                ...
                print("[BrainOS] Failed to auto-connect to '\(provider.name)': \(error)")
            }
        }
    }
    ...
    public func testConnection(
        ...
        guard let url = tempProvider.url(for: "/models") else {
            print("[BrainOS] Test Connection: Invalid URL")
            throw RemoteProviderError.invalidURL
        }

        print("[BrainOS] Test Connection: Requesting \(url.absoluteString)")
        ...
        for (key, value) in testHeaders {
            ...
            if key.lowercased() == "authorization" || key.lowercased() == "x-api-key" {
                print("[BrainOS] Test Connection: Adding header \(key)=***")
            } else {
                print("[BrainOS] Test Connection: Adding header \(key)=\(value)")
            }
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("[BrainOS] Test Connection: Invalid response type")
                throw RemoteProviderError.connectionFailed("Invalid response")
            }

            print("[BrainOS] Test Connection: HTTP \(httpResponse.statusCode)")

            if httpResponse.statusCode >= 400 {
                let errorMessage = String(data: data, encoding: .utf8) ?? "Unknown error"
                print("[BrainOS] Test Connection: Error response: \(errorMessage)")
                throw RemoteProviderError.connectionFailed("HTTP \(httpResponse.statusCode): \(errorMessage)")
            }

            // Parse models response
            let modelsResponse = try JSONDecoder().decode(ModelsResponse.self, from: data)
            print("[BrainOS] Test Connection: Success - found \(modelsResponse.data.count) models")
            return modelsResponse.data.map { $0.id }
        } catch let error as RemoteProviderError {
            throw error
        } catch {
            print("[BrainOS] Test Connection: Network error: \(error)")
        ...
        -> [String]
    {
        guard let url = tempProvider.url(for: "/messages") else {
            print("[BrainOS] Test Connection (Anthropic): Invalid URL")
            throw RemoteProviderError.invalidURL
        }

        print("[BrainOS] Test Connection (Anthropic): Requesting \(url.absoluteString)")
        ...
        for (key, value) in testHeaders {
            if key.lowercased() == "x-api-key" {
                print("[BrainOS] Test Connection (Anthropic): Adding header \(key)=***")
            } else {
                print("[BrainOS] Test Connection (Anthropic): Adding header \(key)=\(value)")
            }
            request.setValue(value, forHTTPHeaderField: key)
        }
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("[BrainOS] Test Connection (Anthropic): Invalid response type")
                throw RemoteProviderError.connectionFailed("Invalid response")
            }

            print("[BrainOS] Test Connection (Anthropic): HTTP \(httpResponse.statusCode)")

            // 401 = invalid API key
            if httpResponse.statusCode == 401 {
                let errorMessage = String(data: data, encoding: .utf8) ?? "Invalid API key"
                print("[BrainOS] Test Connection (Anthropic): Auth error: \(errorMessage)")
            ...
            if httpResponse.statusCode >= 500 {
                ...
                print("[BrainOS] Test Connection (Anthropic): Server error: \(errorMessage)")
                throw RemoteProviderError.connectionFailed("HTTP \(httpResponse.statusCode): \(errorMessage)")
            }

            // Any 2xx or 4xx (except 401) means the connection works
            print(
                "[BrainOS] Test Connection (Anthropic): Success - returning \(Self.anthropicModels.count) known models"
            )
            return Self.anthropicModels
        } catch let error as RemoteProviderError {
            throw error
        } catch {
            print("[BrainOS] Test Connection (Anthropic): Network error: \(error)")
            throw RemoteProviderError.connectionFailed(error.localizedDescription)
        }
    }

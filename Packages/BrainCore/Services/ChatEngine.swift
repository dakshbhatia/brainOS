...
actor ChatEngine: Sendable, ChatEngineProtocol {
    ...
    private func wrapStreamWithLogging(
        ...
        let producerTask = Task.detached(priority: .userInitiated) {
            ...
            print("[BrainOS][Stream] Starting stream wrapper for model: \(model)")

            do {
                for try await delta in inner {
                    // Check for task cancellation to allow early termination
                    if Task.isCancelled {
                        print("[BrainOS][Stream] Task cancelled after \(deltaCount) deltas")
                        continuation.finish()
                        return
                    }
                    deltaCount += 1
                    ...
                    if deltaCount % 50 == 1 || timeSinceLastDelta > 2.0 {
                        print(
                            "[BrainOS][Stream] Delta #\(deltaCount): +\(String(format: "%.2f", timeSinceStart))s total, gap=\(String(format: "%.3f", timeSinceLastDelta))s, len=\(delta.count)"
                        )
                    }
                ...
                print(
                    "[BrainOS][Stream] Stream completed: \(deltaCount) deltas in \(String(format: "%.2f", totalTime))s"
                )

                continuation.finish()
            } catch let inv as ServiceToolInvocation {
                print("[BrainOS][Stream] Tool invocation: \(inv.toolName)")
                ...
                if Task.isCancelled || error is CancellationError {
                    print("[BrainOS][Stream] Stream cancelled after \(deltaCount) deltas")
                    continuation.finish()
                    return
                }
                print("[BrainOS][Stream] Stream error after \(deltaCount) deltas: \(error.localizedDescription)")
                finishReason = .error
        ...
        continuation.onTermination = { @Sendable termination in
            switch termination {
            case .cancelled:
                print("[BrainOS][Stream] Consumer cancelled - stopping producer task")
                producerTask.cancel()
            case .finished:

...
public final class MCPProviderManager: ObservableObject {
    ...
    public func connect(providerId: UUID) async throws {
        ...
        do {
            ...
            if var updatedState = providerStates[providerId] {
                ...
                print(
                    "[BrainOS] MCP Provider '\(provider.name)': Connected with \(updatedState.discoveredToolCount) tools"
                )
            }
            notifyStatusChanged()
            ...
            print("[BrainOS] MCP Provider '\(provider.name)': Connection failed - \(error)")
            notifyStatusChanged()
            throw error
        }
    }
    ...
    public func disconnect(providerId: UUID) {
        ...
        if let provider = configuration.provider(id: providerId) {
            print("[BrainOS] MCP Provider '\(provider.name)': Disconnected")
        }

...
public actor RemoteProviderService: ToolCapableService {
                        ...
                        if line.hasPrefix("data: ") {
                            ...
                            if let jsonData = dataContent.data(using: .utf8) {
                                do {
                                    ...
                                    print(
                                        "[BrainOS] Warning: Failed to parse SSE chunk in streamDeltas: \(error.localizedDescription)"
                                    )
                                }
                            }
                        }
    ...
    func streamWithTools(
        ...
        let producerTask = Task {
            do {
                    ...
                    while let newlineIndex = buffer.firstIndex(of: "\n") {
                        ...
                        if line.hasPrefix("data: ") {
                            ...
                            if dataContent.trimmingCharacters(in: .whitespaces) == "[DONE]" {
                                ...
                                {
                                    print(
                                        "[BrainOS] Stream [DONE]: Emitting accumulated tool call '\(toolName)' with \(firstToolCall.value.args.count) bytes of arguments"
                                    )
                                    continuation.finish(
                                        throwing: ServiceToolInvocation(
                                            toolName: toolName,
                            ...
                            if let jsonData = dataContent.data(using: .utf8) {
                                do {
                                    if providerType == .anthropic {
                                        ...
                                        if let eventType = try? JSONDecoder().decode(
                                            ...
                                            case "content_block_start":
                                                if let startEvent = try? JSONDecoder().decode(
                                                    ...
                                                    if case .toolUse(let toolBlock) = startEvent.content_block {
                                                        ...
                                                        print(
                                                            "[BrainOS] Tool call detected: index=\(idx), name=\(toolBlock.name)"
                                                        )
                                                    }
                                                }
                                            case "message_delta":
                                            ...
                                            case "message_stop":
                                                ...
                                                {
                                                    print(
                                                        "[BrainOS] Anthropic stream ended: Emitting tool call '\(toolName)'"
                                                    )
                                                    continuation.finish(
                                                        throwing: ServiceToolInvocation(
                                                            toolName: toolName,
                                        ...
                                        if let toolCalls = chunk.choices.first?.delta.tool_calls {
                                            for toolCall in toolCalls {
                                                ...
                                                if let name = toolCall.function?.name {
                                                    current.name = name
                                                    print("[BrainOS] Tool call detected: index=\(idx), name=\(name)")
                                                }
                                                if let args = toolCall.function?.arguments {
                                                    current.args += args
                                                }
                                            ...
                                            !finishReason.isEmpty
                                        {
                                            ...
                                            print(
                                                "[BrainOS] Received finish_reason: '\(finishReason)', accumulated tool calls: \(accumulatedToolCalls.count)"
                                            )
                                            ...
                                            if !accumulatedToolCalls.isEmpty {
                                                ...
                                                {
                                                    print(
                                                        "[BrainOS] Emitting tool call '\(toolName)' on finish_reason '\(finishReason)'"
                                                    )
                                                    continuation.finish(
                                                        throwing: ServiceToolInvocation(
                                                            toolName: toolName,
                                    ...
                                    print(
                                        "[BrainOS] Warning: Failed to parse SSE chunk: \(error.localizedDescription)"
                                    )
                                    print("[BrainOS] Raw chunk data: \(dataContent.prefix(500))")
                                }
                            }
                        }
                    }
                ...
                {
                    print(
                        "[BrainOS] Stream ended: Emitting accumulated tool call '\(toolName)' (finish_reason was: \(lastFinishReason ?? "none"))"
                    )
                    continuation.finish(
                        throwing: ServiceToolInvocation(
                            toolName: toolName,
                ...
                if Task.isCancelled {
                    ...
                    print("[BrainOS] Stream error: \(error.localizedDescription)")
                    continuation.finish(throwing: error)
                }
            }
        }
    ...
    private func buildURLRequest(for request: RemoteChatRequest) throws -> URLRequest {
        ...
        if let jsonString = String(data: bodyData, encoding: .utf8) {
            print("[BrainOS] Remote Provider (\(provider.providerType.rawValue)) Request Body:\n\(jsonString)")
        }

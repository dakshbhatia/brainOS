...
class ThemeManager: ObservableObject {
    ...
    private init() {
        print("[BrainOS] ThemeManager: Initializing...")
        ...
        print("[BrainOS] ThemeManager: Found \(loadedThemes.count) installed themes")
        ...
        if let customTheme = ThemeConfigurationStore.loadActiveTheme() {
            print("[BrainOS] ThemeManager: Restoring active theme '\(customTheme.metadata.name)'")
            ...
            if let theme = builtInTheme {
                print("[BrainOS] ThemeManager: Using built-in '\(theme.metadata.name)' theme (auto)")
                let themeInstance = CustomizableTheme(config: theme)
                self.currentTheme = themeInstance
                self.chatTheme = themeInstance
            } else {
                // Fallback to default CustomTheme if built-in themes aren't installed
                print("[BrainOS] ThemeManager: No built-in theme found, using fallback")
                let fallbackTheme =
        ...
        print("[BrainOS] ThemeManager: Initialization complete")
    }
    ...
    func refreshInstalledThemes() {
        ...
        print("[BrainOS] ThemeManager: Refreshed themes, found \(installedThemes.count) themes")
    }

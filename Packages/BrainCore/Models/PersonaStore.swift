...
public enum PersonaStore {
    ...
    public static func loadAll() -> [Persona] {
        ...
        for file in files where file.pathExtension == "json" {
            do {
                ...
                print("[BrainOS] Failed to load persona from \(file.lastPathComponent): \(error)")
            }
        }

        // Sort: built-ins first (Default, then BrainOS), then custom by name
        return personas.sorted { a, b in
            if a.isBuiltIn && !b.isBuiltIn { return true }
            if !a.isBuiltIn && b.isBuiltIn { return false }
            if a.isBuiltIn && b.isBuiltIn {
                // Default comes before BrainOS
    ...
    public static func load(id: UUID) -> Persona? {
        ...
        do {
            ...
            print("[BrainOS] Failed to load persona \(id): \(error)")
            return nil
        }
    }
    ...
    public static func save(_ persona: Persona) {
        ...
        guard !persona.isBuiltIn else {
            print("[BrainOS] Cannot save built-in persona: \(persona.name)")
            return
        }
        ...
        do {
            ...
            print("[BrainOS] Failed to save persona \(persona.id): \(error)")
        }
    }
    ...
    public static func delete(id: UUID) -> Bool {
        ...
        if Persona.builtInPersonas.contains(where: { $0.id == id }) {
            print("[BrainOS] Cannot delete built-in persona")
            return false
        }
        ...
        do {
            ...
            print("[BrainOS] Failed to delete persona \(id): \(error)")
            return false
        }
    }

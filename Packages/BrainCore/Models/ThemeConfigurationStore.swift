...
public enum ThemeConfigurationStore {
    ...
    static func loadActiveThemeId() -> UUID? {
        ...
        do {
            ...
            print("[BrainOS] Failed to load active theme reference: \(error)")
            return nil
        }
    }
    ...
    static func saveActiveThemeId(_ themeId: UUID?) {
        ...
        do {
            ...
            if let themeId = themeId {
                ...
                print("[BrainOS] Saved active theme ID: \(themeId)")
            } else {
                // Remove active theme file if set to nil
                try? FileManager.default.removeItem(at: url)
                print("[BrainOS] Cleared active theme")
            }
        } catch {
            print("[BrainOS] Failed to save active theme reference: \(error)")
        }
    }
    ...
    static func listThemes() -> [CustomTheme] {
        ...
        guard FileManager.default.fileExists(atPath: themesDir.path) else {
            print("[BrainOS] Themes directory does not exist: \(themesDir.path)")
            return []
        }
        ...
        do {
            ...
            print("[BrainOS] Found \(themeFiles.count) theme files in \(themesDir.path)")
            ...
            let themes = themeFiles.compactMap { url -> CustomTheme? in
                do {
                    ...
                    print("[BrainOS] Failed to load theme from \(url.lastPathComponent): \(error)")
                    // Try to recover by deleting corrupted file and reinstalling if built-in
                    handleCorruptedThemeFile(url)
                    return nil
                }
            }
            print("[BrainOS] Successfully loaded \(themes.count) themes")
            return themes
        } catch {
            print("[BrainOS] Failed to list themes: \(error)")
            return []
        }
    }
    ...
    static func loadTheme(id: UUID) -> CustomTheme? {
        ...
        do {
            ...
            print("[BrainOS] Failed to load theme \(id): \(error)")
            return nil
        }
    }
    ...
    static func saveTheme(_ theme: CustomTheme) {
        ...
        do {
            ...
            try data.write(to: url, options: [.atomic])
            print("[BrainOS] Saved theme '\(theme.metadata.name)' to \(url.lastPathComponent)")
        } catch {
            print("[BrainOS] Failed to save theme \(theme.metadata.name): \(error)")
        }
    }
    ...
    static func deleteTheme(id: UUID) -> Bool {
        ...
        if let theme = loadTheme(id: id), theme.isBuiltIn {
            print("[BrainOS] Cannot delete built-in theme: \(theme.metadata.name)")
            return false
        }
        ...
        do {
            if FileManager.default.fileExists(atPath: url.path) {
                try FileManager.default.removeItem(at: url)
                print("[BrainOS] Deleted theme: \(id)")
            }
            ...
            print("[BrainOS] Failed to delete theme \(id): \(error)")
            return false
        }
    }
    ...
    static func installBuiltInThemesIfNeeded() {
        do {
            try ensureDirectoryExists(themesDirectoryURL())
            print("[BrainOS] Themes directory: \(themesDirectoryURL().path)")
        } catch {
            print("[BrainOS] Failed to create themes directory: \(error)")
            return
        }
        ...
        for theme in CustomTheme.allBuiltInPresets {
            let url = themeFileURL(for: theme.metadata.id)
            if !FileManager.default.fileExists(atPath: url.path) {
                print("[BrainOS] Installing built-in theme: \(theme.metadata.name)")
                saveTheme(theme)
            }
        }
    ...
    static func forceReinstallBuiltInThemes() {
        print("[BrainOS] Force reinstalling all built-in themes...")
        do {
            try ensureDirectoryExists(themesDirectoryURL())
        } catch {
            print("[BrainOS] Failed to create themes directory: \(error)")
        ...
        for theme in CustomTheme.allBuiltInPresets {
            print("[BrainOS] Reinstalling built-in theme: \(theme.metadata.name)")
            saveTheme(theme)
        }
        builtInThemesInstalled = true
    }
    ...
    private static func handleCorruptedThemeFile(_ url: URL) {
        ...
        if let builtInTheme = CustomTheme.allBuiltInPresets.first(where: { $0.metadata.id == uuid }) {
            print("[BrainOS] Reinstalling corrupted built-in theme: \(builtInTheme.metadata.name)")
            try? FileManager.default.removeItem(at: url)
            saveTheme(builtInTheme)
        } else {
            // Custom theme - just delete the corrupted file
            print("[BrainOS] Removing corrupted custom theme file: \(filename)")
            try? FileManager.default.removeItem(at: url)
        }
    }
    ...
    private static func baseDirectoryURL() -> URL {
        ...
        let bundleId = Bundle.main.bundleIdentifier ?? "com.BrainOS.app"
    ...
    private static func ensureDirectoryExists(_ url: URL) throws {
        ...
        if !FileManager.default.fileExists(atPath: url.path, isDirectory: &isDir) {
            ...
            print("[BrainOS] Created directory: \(url.path)")
        }
    }
}

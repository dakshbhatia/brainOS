                            ...
                            SettingsSection(title: "System", icon: "gear") {
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    SettingsToggle(
                                        ...
                                        description: "Launch BrainOS when you sign in",
                                    ...
                                    SettingsSubsection(label: "Command Line Tool") {
                                        VStack(alignment: .leading, spacing: 12) {
                                            Text("Install the `BrainOS` CLI into your PATH for terminal access.")
                                                .font(.system(size: 12))
...
extension ConfigurationView {
    private func installCLI() {
        ...
        if tryInstall(cliURL: cliURL, into: brewBin) {
            ...
            cliInstallMessage = "Installed to \(brewBin.appendingPathComponent("BrainOS").path)"
            return
        }

        if tryInstall(cliURL: cliURL, into: usrLocalBin) {
            cliInstallSuccess = true
            cliInstallMessage = "Installed to \(usrLocalBin.appendingPathComponent("BrainOS").path)"
            return
        }
        ...
        if tryInstall(cliURL: cliURL, into: userLocalBin) {
            let linkPath = userLocalBin.appendingPathComponent("BrainOS").path
    ...
    private func resolveCLIExecutableURL() -> URL? {
        ...
        let helpers = appURL.appendingPathComponent("Contents/Helpers/BrainOS", isDirectory: false)
        ...
        let macOS = appURL.appendingPathComponent("Contents/MacOS/BrainOS", isDirectory: false)
        ...
        // Check for BrainOS-cli binary (the actual CLI product name)
        let debugCLI = productsDir.appendingPathComponent("BrainOS-cli", isDirectory: false)
        if fm.fileExists(atPath: debugCLI.path), fm.isExecutableFile(atPath: debugCLI.path) {
            return debugCLI
        }

        // Check for BrainOS binary in Products (might be named this in some builds)
        let debugBrainOS = productsDir.appendingPathComponent("BrainOS", isDirectory: false)
        if fm.fileExists(atPath: debugBrainOS.path), fm.isExecutableFile(atPath: debugBrainOS.path) {
            return debugBrainOS
        }

        // Check Release folder
        let releaseDir = productsDir.deletingLastPathComponent().appendingPathComponent("Release")
        let releaseCLI = releaseDir.appendingPathComponent("BrainOS-cli", isDirectory: false)
        if fm.fileExists(atPath: releaseCLI.path), fm.isExecutableFile(atPath: releaseCLI.path) {
            return releaseCLI
        }

        let releaseBrainOS = releaseDir.appendingPathComponent("BrainOS", isDirectory: false)
        if fm.fileExists(atPath: releaseBrainOS.path), fm.isExecutableFile(atPath: releaseBrainOS.path) {
            return releaseBrainOS
        }

        // 4. Check inside Release app bundle's Helpers folder
        let releaseAppHelpers =
            releaseDir
            .appendingPathComponent("BrainOS.app/Contents/Helpers/BrainOS", isDirectory: false)
        ...
        let releaseAppMacOS =
            releaseDir
            .appendingPathComponent("BrainOS.app/Contents/MacOS/BrainOS", isDirectory: false)
    ...
    private func tryInstall(cliURL: URL, into dir: URL) -> Bool {
        ...
        let linkURL = dir.appendingPathComponent("BrainOS")

            ...
            do {
                    ...
                    for (index, t) in contextTurns.enumerated() {
                        ...
                        case .assistant:
                            ...
                            if t.content.isEmpty && (t.toolCalls == nil || t.toolCalls!.isEmpty) {
                                ...
                                print("[BrainOS] Warning: Skipping empty assistant message at index \(index)")
                                continue
                            }
                ...
                outer: while attempts < maxAttempts {
                    ...
                    do {
                        ...
                        var uiDeltaCount = 0
                        print("[BrainOS][UI] Starting stream consumption on MainActor")
                        ...
                        print(
                            "[BrainOS][UI] Stream consumption completed: \(uiDeltaCount) deltas in \(String(format: "%.2f", totalTime))s, final contentLen=\(assistantTurn.content.count)"
                        )
                        ...
                        do {
                            ...
                            print(
                                "[BrainOS][Tool] Executing: \(inv.toolName) with args: \(truncatedArgs)\(inv.jsonArguments.count > 200 ? "..." : "")"
                            )
                            ...
                            print(
                                "[BrainOS][Tool] Success: \(inv.toolName) returned \(resultText.count) chars: \(truncatedResult)\(resultText.count > 500 ? "..." : "")"
                            )
                        } catch {
                            // Log tool error
                            print("[BrainOS][Tool] Error: \(inv.toolName) failed: \(error.localizedDescription)")

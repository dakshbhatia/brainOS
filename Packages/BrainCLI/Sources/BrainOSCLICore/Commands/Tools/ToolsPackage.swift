...
public struct ToolsPackage {
    public static func execute(args: [String]) {
        // Parse arguments: BrainOS tools package <plugin_id> <version> [dylib_path]
        guard args.count >= 2 else {
            fputs("Usage: BrainOS tools package <plugin_id> <version> [dylib_path]\n", stderr)
        ...
        print("Install with: BrainOS tools install ./\(zipName)")
        exit(EXIT_SUCCESS)
    }
}

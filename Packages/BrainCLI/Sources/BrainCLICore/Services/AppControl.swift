...
//  BrainOS
//
//  Service for controlling the BrainOS app via distributed notifications and launching it if needed.
//
...
public struct AppControl {
    ...
    public static func launchAppIfNeeded() async {
        ...
        do {
            ...
            openByBundle.arguments = ["-n", "-b", "com.dinoki.BrainOS", "--args", "--launched-by-cli"]
        ...
        if !launched {
            fputs(
                "Could not launch BrainOS.app. Install it with Homebrew: brew install --cask BrainOS\n",
                stderr
            )
            return
        }
    ...
    /// Attempts to locate the installed BrainOS.app bundle path using common locations and Spotlight.
    private static func findAppBundlePath() -> String? {
        ...
        if let path = spotlightFind(queryArgs: [
            ...
            "kMDItemCFBundleIdentifier == 'com.dinoki.BrainOS'",
        ]) {

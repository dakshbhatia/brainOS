...
//  BrainOS
//
//  Command to start the BrainOS server with optional port configuration and network exposure settings.
//
...
public struct ServeCommand: Command {
    ...
    public static func execute(args: [String]) async {
        ...
        if expose && !assumeYes {
            ...
            let warning = """
                WARNING: Exposing BrainOS to the local network will allow other devices on your LAN
        ...
        AppControl.postDistributedNotification(
            name: "com.dinoki.BrainOS.control.serve",
            userInfo: buildInfo()
        )
        ...
        fputs(
            ...
            Hints:
              - The port may be busy. Try: BrainOS serve --port \(altPort)
              - Ensure BrainOS.app is installed: brew install --cask BrainOS
              - Try launching the app first, then serve:
                open -a /Applications/BrainOS.app && BrainOS serve
              - You can also open the UI: BrainOS ui
            """.appending("\n"),
            stderr
        )
        exit(EXIT_FAILURE)

...
public struct ToolsCreate {
    public static func execute(args: [String]) {
        guard let name = args.first, !name.isEmpty else {
            fputs("Usage: BrainOS tools create <name> [--language swift|rust]\n", stderr)
            exit(EXIT_FAILURE)
        }
        var language = "swift"
    ...
    private static func createSwiftPlugin(name: String) {
        ...
        let pluginSwift = """
            ...
            @_cdecl("BrainOS_plugin_entry")
            public func BrainOS_plugin_entry() -> UnsafeRawPointer? {
        ...
        let readme = """
            ...
            2. Package (for distribution):
               ```bash
               BrainOS tools package dev.example.\(name) 0.1.0
               ```
               This creates `dev.example.\(name)-0.1.0.zip`.
               
            3. Install locally:
               ```bash
               BrainOS tools install ./dev.example.\(name)-0.1.0.zip
               ```
            ...
            1. Package it with the correct naming convention:
               ```bash
               BrainOS tools package <plugin_id> <version>
               ```
    ...
    private static func createRustPlugin(name: String) {
        ...
        let readme = """
            ...
            This is a placeholder for a Rust-based BrainOS plugin. Build a cdylib exposing
            `BrainOS_plugin_entry` that returns the generic ABI table.
            """
        try? readme.write(to: dir.appendingPathComponent("README.md"), atomically: true, encoding: .utf8)
    }
}

...
public struct ToolsInstall {
    public static func execute(args: [String]) async {
        guard let src = args.first, !src.isEmpty else {
            fputs("Usage: BrainOS tools install <plugin_id|url-or-path> [--version <semver>]\n", stderr)
            exit(EXIT_FAILURE)
        }
    ...
    private static func installFromRegistry(pluginId: String, args: [String]) async {
        ...
        do {
            ...
            AppControl.postDistributedNotification(name: "com.dinoki.BrainOS.control.toolsReload", userInfo: [:])
            exit(EXIT_SUCCESS)
        } catch {

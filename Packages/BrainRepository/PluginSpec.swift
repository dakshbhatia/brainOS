...
public struct PluginRequirements: Codable, Equatable, Sendable {
    public let BrainOS_min_version: SemanticVersion?
}
...
public extension PluginSpec {
    func resolveBestVersion(
        ...
        targetArch: CPUArch,
        minimumBrainOSVersion: SemanticVersion?,
        ...
        let filtered: [PluginVersionEntry] = versions.filter { entry in
            guard let req = entry.requires?.BrainOS_min_version,
                let min = minimumBrainOSVersion
            else { return true }
            return min >= req
        }

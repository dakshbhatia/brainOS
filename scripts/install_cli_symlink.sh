...
# Creates/updates a convenient `BrainOS` symlink to either:
#   1) the app's embedded CLI at BrainOS.app/Contents/MacOS/BrainOS, or
#   2) a locally built CLI binary in DerivedData (dev mode).
#
# Usage:
#   scripts/install_cli_symlink.sh [--dev] [--prefix <dir>] [<path-to-BrainOS.app>]
#
# Notes:
...
resolve_cli_from_app() {
  ...
  local candidate
  for candidate in \
    "$app_path/Contents/Helpers/BrainOS" \
    "$app_path/Contents/MacOS/BrainOS"
  do
    if [[ -x "$candidate" ]]; then
      echo "$candidate"
      return 0
...
resolve_cli_from_dev() {
  ...
  for candidate in \
    "$base/BrainOS" \
    "$base/BrainOS-cli" \
    "$base/BrainOS.app/Contents/Helpers/BrainOS" \
    "$base/BrainOS.app/Contents/MacOS/BrainOS" \
    "$REPO_ROOT/build/DerivedData/Build/Products/Debug/BrainOS" \
    "$REPO_ROOT/build/DerivedData/Build/Products/Debug/BrainOS-cli" \
    "$REPO_ROOT/build/DerivedData/Build/Products/Debug/BrainOS.app/Contents/Helpers/BrainOS" \
    "$REPO_ROOT/build/DerivedData/Build/Products/Debug/BrainOS.app/Contents/MacOS/BrainOS"
  do
    if [[ -x "$candidate" ]]; then
      echo "$candidate"
      return 0
...
if [[ "$DEV_MODE" == "1" ]]; then
  ...
  if [[ -z "$APP_PATH" ]]; then
    CANDIDATES=(
      "/Applications/BrainOS.app"
      "$HOME/Applications/BrainOS.app"
      "/Applications/BrainOS.app"
      "$HOME/Applications/BrainOS.app"
    )
    ...
    if CLI_SRC="$(resolve_cli_from_app "$APP_PATH")"; then :; else
      ...
      echo "Expected at: $APP_PATH/Contents/MacOS/BrainOS" >&2
      exit 1
    fi
  fi

  if [[ -z "$CLI_SRC" ]]; then
    echo "Could not locate BrainOS.app automatically. Provide the path explicitly." >&2
    echo "Example: scripts/install_cli_symlink.sh '/Applications/BrainOS.app'" >&2
    exit 1
  fi
fi

TARGET_DIR="$(resolve_target_bin_dir)"
TARGET_LINK="$TARGET_DIR/BrainOS"

...
1. Open `BrainOS.xcworkspace` in Xcode 16.4+
2. Select the `BrainOS` target and press Run
...
- Prompt templating is handled internally by MLX `ChatSession`. BrainOS does not assemble prompts manually.
...
- Add or update tests in `BrainOSTests/` where reasonable
...
2. **Update the README** — If the feature should be highlighted:
   ...
   - Update "What is BrainOS?" if it's a major feature
...
- **[Good First Issues](https://github.com/dinoki-ai/BrainOS/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22)** — Great starting points

Thank you for helping make BrainOS better!
